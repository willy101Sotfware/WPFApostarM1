...
{
    /// <summary>
    /// Lógica de interacción para ReturnMoneyReUC.xaml
    /// </summary>
    public partial class ReturnMoneyReUC : UserControl
    {
        ...
        private async void OnCashDispensed(decimal totalDispensed, Dictionary<int, int> details)
        {
            ...
            AdminPayPlus.SaveLog("ReturnMoneyReUC", "entrando a la ejecucion OnCashDispensed", "OK", totalDispensed.ToString(), transaction);
            ...
            {
                NotifyStatus = false;
                AdminPayPlus.SaveLog("ReturnMoneyReUC", "OnCashDispensed, el estado de la transaccion es Cancelada", "OK", "", transaction);

                if (transaction.Payment.ValorDispensado < transaction.Payment.ValorIngresado)
                {
                    AdminPayPlus.SaveLog("ReturnMoneyReUC", "OnCashDispensed, entrando a la condicion transaction.Payment.ValorDispensado < transaction.Payment.ValorIngresado", "OK", "", transaction);
                ...
                {
                    AdminPayPlus.SaveLog("ReturnMoneyReUC", "OnCashDispensed, entrando a la condiciontransaction.Payment.ValorDispensado == transaction.Payment.ValorIngresado", "OK", "", transaction);
            ...
            {
                AdminPayPlus.SaveLog("ReturnMoneyReUC", "OnCashDispensed, el estado de la transaccion es Aprobado", "OK", "", transaction);
                NotifyStatus = true;
                ...
                else if (transaction.Payment.ValorDispensado == transaction.Payment.ValorSobrante)
                {
                    ...
                    AdminPayPlus.SaveLog("ReturnMoneyReUC", "OnCashDispensed, entrando a la condicion transaction.Payment.ValorDispensado == transaction.Payment.ValorSobrante", "OK", "", transaction);
        ...
        public void savepay(bool notify)
        {
            ...
            if (notify)
            {
                AdminPayPlus.SaveLog("ReturnMoneyReUC", "SavePay, transaccion aprobada, navegando al SuccesRecaudo", "OK", "", transaction);
            ...
            {
                AdminPayPlus.SaveLog("ReturnMoneyReUC", "SavePay, transaccion Cancelada, ", "OK", "", transaction);
                ...
                if (!Utilities.GetConfiguration("noPeripherals").Equals("true"))
                {
                    AdminPayPlus.SaveLog("ReturnMoneyReUC", "FinishCancelNotPay", "OK", string.Concat("ID Transaccion:", transaction.IdTransactionAPi, "/n", "Estado Transaccion:", transaction.StatePay.ToString(), "/n", "Monto:", transaction.Amount.ToString(), "/n", "Valor Dispensado:", transaction.Payment.ValorDispensado.ToString(), "/n", "Valor Ingresado:", transaction.Payment.ValorIngresado.ToString()), transaction);
                    AdminPayPlus.UpdateTransaction(transaction);
                }

                AdminPayPlus.SaveLog("ReturnMoneyReUC", "Saliendo de la ejecucion savepay false", "OK", "", transaction);
